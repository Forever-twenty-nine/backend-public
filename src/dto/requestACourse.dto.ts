export interface CreateRequestACourseDTO {
  name: string;
  company: string;
  email: string;
  phonePrefix: string;
...
export function validateCreateRequestACourseDTO(data: any): {
  isValid: boolean;
  errors: string[];
  data?: CreateRequestACourseDTO;
} {
  const errors: string[] = [];
  ...
  if (!data.phonePrefix || typeof data.phonePrefix !== 'string' || data.phonePrefix.trim() === '') {
