...
const mockService = IWantToTrainService as jest.MockedClass<typeof IWantToTrainService>;
...
describe('IWantToTrainController', () => {
  ...
  beforeEach(() => {
    const mockRepoInstance = new mockRepository({} as any) as jest.Mocked<IWantToTrainRepository>;
    mockServiceInstance = new mockService(mockRepoInstance) as jest.Mocked<IWantToTrainService>;
  ...
  describe('createIWantToTrain', () => {
    it('should create IWantToTrain successfully', async () => {
      ...
      await controller.createIWantToTrain(mockReq as Request, mockRes as Response, mockNext);

      expect(mockServiceInstance.create).toHaveBeenCalledWith(iWantToTrainData);
      expect(mockRes.status).toHaveBeenCalledWith(201);
      expect(mockRes.json).toHaveBeenCalledWith({
        status: 201,
        message: 'IWantToTrain created successfully',
