...
export default class RequestACourseController {
   ...
   */
  createRequestACourse = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      const newRequestACourse = await this.requestACourseService.create(validation.data!);
      res.status(201).json(prepareResponse(201, 'RequestACourse creado exitosamente', newRequestACourse));
    } catch (error) {
