...
export default class IWantToTrainController {
   ...
   */
  createIWantToTrain = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      const newIWantToTrain = await this.iWantToTrainService.create(validation.data!);
      res.status(201).json(prepareResponse(201, 'IWantToTrain creado exitosamente', newIWantToTrain));
    } catch (error) {
